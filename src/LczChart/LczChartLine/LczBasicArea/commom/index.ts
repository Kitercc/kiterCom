...
import { BasicAreaProps, AreaDataSeries, AreaGlobalConfig, AreaSeriesConfig } from '../type'
import {
...
// 生成全局options
export function getAreaGlobalOptions(config: AreaGlobalConfig) {
  const { bgColor, margin, rotationAnimate } = config
  const options: any = {
    grid: {
      left: margin?.l,
      bottom: margin?.b,
  ...
  if (rotationAnimate?.display) {
    options.dataZoom = [
      {
        type: 'inside',
        disabled: !rotationAnimate.dragPan,
        zoomOnMouseWheel: false,
        startValue: 0,
        endValue: (rotationAnimate.showNumer || 6) - 1
      }
    ]
  }
  return options
...
//生成图例系列options
export const getAreaLegendOptions = (config: GeneralLegend, dataMemo: any, dataSeries: AreaDataSeries[]) => {
  let options: any = {}
  if (config.display) {
...
function getAreaSeriesData(data) {
...
export const getAreaSeriesOptions = (
  config: BasicAreaProps,
  dataMemo: any,
  colors: any[],
  ...
  const {
    ...
    emphasis = false,
    stackSeries = []
  } = (config.seriesConfig || {}) as AreaSeriesConfig
  ...
  for (const key in dataMemo) {
    if (Object.prototype.hasOwnProperty.call(dataMemo, key)) {
      ...
      const _data = getAreaSeriesData(item)
      const areaStyle = dataSeries[i]?.areaStyle || defaultAreaStyle //区域样式
      const brokenLine = dataSeries[i]?.brokenLine || deafultBrokenLineValue //折线
      const dataMarker = dataSeries[i]?.dataMarker || defaulAreaDataMarkerValue //数据标记
      ...
      const _series: any = {
        ...
        areaStyle: {
          color: areaStyle.areaSyncColor ? color : getEchartColor(areaStyle.color),
