...
import { DynamicLineProps, LineDataSeries, LineGlobalConfig, LineSeriesConfig } from '../type'
...
export function getLineSeriesData(data) {
...
export const getLineSeriesOptions = (config: DynamicLineProps, dataMemo: any, styledsMemo: LineDataSeries[]) => {
  const options: any = {
    series: []
  }
  let i = 0
  const { markSeries = [], markPointSeries = [] } = (config.seriesConfig || {}) as LineSeriesConfig
  ...
  for (const key in dataMemo) {
    if (Object.prototype.hasOwnProperty.call(dataMemo, key)) {
      ...
      const brokenLine = styledsMemo[i]?.brokenLine || deafultBrokenLineValue //折线
      const dataMarker = styledsMemo[i]?.dataMarker || defaultDataMarkerValue //数据标记
      const valueLabel = styledsMemo[i]?.valueLabel || defaultValueLabelValue //数值标签

      const color = getEchartColor(styledsMemo[i]?.brokenLine?.color)
