...
import { CreateChart } from './common/creatChart'
import { LineBarProps, SeriesConfig } from './type'

export default memo(function LczBasicLineBar(props: LineBarProps) {
  ...
  useEffect(() => {
    if (chartCase.current?.myChart) {
      chartCase.current.setConfig(props, chartData, stylesMemo)
    }
  }, [chartCase.current, JSON.stringify(chartData), JSON.stringify(otherProps), JSON.stringify(stylesMemo)])
