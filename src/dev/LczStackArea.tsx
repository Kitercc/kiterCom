...
const LczStackArea = LczChart.LczChartLine.LczStackArea

export const T_LczStackArea = memo(function T_Lcz3dTorus() {
  const c2 = {
    selected: 'single',
    single: '#fc3dfc',
    gradient: {
  ...
  const config: BasicAreaProps = {
    ...
    seriesConfig: {
      dataSeries: [
        {
          map: { fieldName: 'ss', displayName: 'ss' },
          areaStyle: {
            areaSyncColor: true,
            color: c2,
            startOrigin: 'start',
        ...
        {
          map: { fieldName: 'aa', displayName: 'aa' },
          areaStyle: {
            areaSyncColor: true,
            color: c3,
            startOrigin: 'start',
  ...
  return (
    <div style={{ width: 900, height: 600, margin: ' 0 auto' }}>
      ...
      {show && <LczStackArea w={900} h={600} {...config} data={data} onClick={a => console.log(a)} />}
    </div>
  )
})
